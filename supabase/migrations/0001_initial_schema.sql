...
create table if not exists public.profiles (
  ...
  birth_year int,
  email text not null,
  role text not null default 'user' check (role in ('user', 'admin', 'scanner')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint birth_year_valid check (
    birth_year is null
...
create table if not exists public.tickets (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint used_entries_not_more_than_max check (used_entries <= max_entries),
  constraint one_ticket_per_user_party unique (party_id, user_id)
);
...
create policy "Published parties are visible"
on public.parties for select
to anon, authenticated
using (status = 'published' or private.current_user_role() = 'admin');
...
on public.party_images for select
to anon, authenticated
using (
  exists (
    select 1 from public.parties p
    where p.id = party_images.party_id
    and (p.status = 'published' or private.current_user_role() = 'admin')
  )
);

create policy "Admins can manage party images"
on public.party_images for all
to authenticated
using (private.current_user_role() = 'admin')
with check (private.current_user_role() = 'admin');
...
on public.ticket_scans for select
to authenticated
using (scanned_by = auth.uid() or private.current_user_role() = 'admin');

create or replace function private.scan_ticket(
  p_qr_token text,
  p_party_id uuid,
...
begin
  ...
  if v_role not in ('admin', 'scanner') then
    return jsonb_build_object('ok', false, 'reason', 'unauthorized', 'message', 'Bu işlem için yetkiniz yok.');
  end if;

  if p_requested_entries is null or p_requested_entries < 1 then
    return jsonb_build_object('ok', false, 'reason', 'invalid_requested_entries', 'message', 'Giriş kişi sayısı geçersiz.');
  end if;
  ...
  if not found then
    insert into public.ticket_scans (ticket_id, party_id, scanned_by, requested_entries, result, reason)
    values (null, p_party_id, auth.uid(), p_requested_entries, 'rejected', 'ticket_not_found');
    return jsonb_build_object('ok', false, 'reason', 'ticket_not_found', 'message', 'Bilet bulunamadı.');
  end if;

  if v_ticket.party_id <> p_party_id then
    insert into public.ticket_scans (ticket_id, party_id, scanned_by, requested_entries, result, reason)
    values (v_ticket.id, p_party_id, auth.uid(), p_requested_entries, 'rejected', 'wrong_party');
    return jsonb_build_object('ok', false, 'reason', 'wrong_party', 'message', 'Bu bilet bu parti için geçerli değil.');
  end if;

  if v_ticket.status <> 'active' then
    insert into public.ticket_scans (ticket_id, party_id, scanned_by, requested_entries, result, reason)
    values (v_ticket.id, v_ticket.party_id, auth.uid(), p_requested_entries, 'rejected', 'ticket_not_active');
    return jsonb_build_object('ok', false, 'reason', 'ticket_not_active', 'message', 'Bu bilet aktif değil.');
  end if;
  ...
  if v_remaining < p_requested_entries then
    insert into public.ticket_scans (ticket_id, party_id, scanned_by, requested_entries, result, reason)
    values (v_ticket.id, v_ticket.party_id, auth.uid(), p_requested_entries, 'rejected', 'not_enough_entries');
    return jsonb_build_object('ok', false, 'reason', 'not_enough_entries', 'message', 'Biletin kalan giriş hakkı yetersiz.', 'remaining_entries', v_remaining);
  end if;

  update public.tickets
  set used_entries = used_entries + p_requested_entries,
      status = case when used_entries + p_requested_entries >= max_entries then 'used' else status end
  where id = v_ticket.id;

  insert into public.ticket_scans (ticket_id, party_id, scanned_by, requested_entries, result, reason)
  values (v_ticket.id, v_ticket.party_id, auth.uid(), p_requested_entries, 'approved', 'ok');
...
set search_path = public
as $$
  select private.scan_ticket(p_qr_token, p_party_id, p_requested_entries)
$$;

grant usage on schema private to anon, authenticated;
grant execute on function private.current_user_role() to anon, authenticated;
grant execute on function private.scan_ticket(text, uuid, int) to authenticated;
grant execute on function public.scan_ticket(text, uuid, int) to authenticated;
